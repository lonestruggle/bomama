...
/**
 * Per account + starter-status bucket bijhouden hoe vaak dit vastliep.
 * Formaat: rsn|bucket=count|lastEpochMs
 */
public final class StarterStatusLearningStore {

    private static final String GROUP = "combatbot";
    private static final String KEY = "starterStatusLearningBlob";

    private StarterStatusLearningStore() {
    }
    ...
    public static Entry get(CombatBotConfig cfg, String key) {
        if (cfg == null || key == null || key.trim().isEmpty()) return null;
        return parse(cfg.starterStatusLearningBlob()).get(key.trim().toLowerCase(Locale.ROOT));
    }

    public static void increment(ConfigManager cm, CombatBotConfig cfg, String key) {
        if (cm == null || cfg == null || key == null || key.trim().isEmpty()) return;
        long now = System.currentTimeMillis();
        String norm = key.trim().toLowerCase(Locale.ROOT);
        Map<String, Entry> all = parse(cfg.starterStatusLearningBlob());
    ...
    private static Map<String, Entry> parse(String blob) {
        ...
        if (blob == null || blob.trim().isEmpty()) return out;
